...
for i = 1:length(req_params.cell_type)
    ...
    indType = intersect(ind,...
        find(strcmp(req_params.cell_type{i}, cellType)));
    plotHistForFC(accuracy(t,indType),bin_edges);
   
    
end
legend(req_params.cell_type)
sgtitle(['classifier, T: ' num2str(ts(t))])
...
req_params.grade = 7;
req_params.cell_type = {'PC cs','BG msn'};
req_params.task = 'choice';
...
for ii = 1:length(cells)
    ...
    for d = 1:length(DIRECTIONS)
        ...
        ind = find((~boolFail) & match_d(1,:)==DIRECTIONS(d));
